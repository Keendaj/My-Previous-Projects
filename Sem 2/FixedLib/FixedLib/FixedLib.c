...
static char __CalcLength(int a) {
	char length = 0;
	do {
		a /= 10;
		length++;
...
int FixedAsInt(fixed_t c) {
	return c.IntPart;
}

double FixedAsDouble(fixed_t c) {
...
char FixedWrite(fixed_t c, char const* filename) {
...
fixed_t FixedNegate(fixed_t c) {
	fixed_t res = { -1 * c.IntPart,-1 * c.FloatPart,c.IntLength,c.FloatLength };
	return res;
}

fixed_t FixedReciprocal(fixed_t c) {
	fixed_t a = { 1,0,1,0 };
	return FixedDiv(a, c);
}

fixed_t FixedAbs(fixed_t c) {
	int IsNegative = 1;
	if (c.IntPart < 0) {
		IsNegative = 1;
	}
...
fixed_t FixedFloor(fixed_t c) {
	if (c.FloatPart == 0) {
		return c;
	}
	fixed_t b = { c.IntPart,0,c.IntLength,0 };
	return b;
}

fixed_t FixedCeil(fixed_t c) {
	if (c.FloatPart == 0) {
		return c;
	}
